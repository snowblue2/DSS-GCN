...
from models import REGCN, ablation
#import fitlog
...
class Instructor:
    def __init__(self, opt):
        self.opt = opt
        absa_dataset = ABSADatesetReader(dataset=opt.dataset, embed_dim=opt.embed_dim, usebert=opt.usebert)
    ...
    def _evaluate_acc_f1(self):
        ...
        test_indices, test_att = [], []
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                t_inputs = [t_sample_batched[col].to(opt.device) for col in self.opt.inputs_cols]
                t_targets = t_sample_batched['polarity'].to(opt.device)
                t_outputs, indices, att = self.model(t_inputs)
        ...
        return test_acc, f1, test_indices, test_att
    ...
    def run(self, repeats=5):
        ...
        for i in range(repeats):
            ...
            if max_test_acc > max_test_acc_avg:
                ...
                #fitlog.add_best_metric({"test": {"Acc": max_test_acc, "LOSS":max_test_f1}})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--usebert', default=0, type=int)
    ...
    input_colses = {
        ...
        'ablation': ['text_indices', 'aspect_indices', 'left_indices', 'pmi_graph', 'cos_graph', 'dep_graph'],
    }
    initializers = {
