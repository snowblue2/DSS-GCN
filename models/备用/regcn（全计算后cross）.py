    ...
    def forward(self, inputs):
        ...
        for i in range(num_layer):
            if f_n is None:
                ...
                x_d_pmi_1 = F.relu(self.gc3(self.position_weight(x_pmi_2, aspect_double_idx, text_len, aspect_len), dep_adj))
                x_d_pmi_2 = F.relu(self.gc4(self.position_weight(x_d_pmi_1, aspect_double_idx, text_len, aspect_len), dep_adj))
                ...
                x_d_cos_1 = F.relu(self.gc7(self.position_weight(x_cos_2, aspect_double_idx, text_len, aspect_len), dep_adj))
                x_d_cos_2 = F.relu(self.gc8(self.position_weight(x_d_cos_1, aspect_double_idx, text_len, aspect_len), dep_adj))
                f_n = torch.cat([(0.3*x_pmi_2 + x_d_pmi_2) ,  (0.3*x_cos_2 + x_d_cos_2)],dim=2)
            else:#多层的更新
                x_pmi, x_cos = self.cross_network(f0,f_n)
                x_pmi = F.relu(self.ggc1(self.position_weight(x_pmi, aspect_double_idx, text_len, aspect_len), pmi_adj))
                x_d_pmi = F.relu(self.ggc2(self.position_weight(x_pmi, aspect_double_idx, text_len, aspect_len), dep_adj))
                x_cos = F.relu(self.ggc3(self.position_weight(x_cos, aspect_double_idx, text_len, aspect_len),cos_adj))
                x_d_cos = F.relu(self.ggc4(self.position_weight(x_cos, aspect_double_idx, text_len, aspect_len), dep_adj))
                f_n = torch.cat([(0.3*x_pmi + x_d_pmi) ,  (0.3*x_cos + x_d_cos)],dim=2)

        # x_pmi = F.relu(self.gc1(self.position_weight(text_out, aspect_double_idx, text_len, aspect_len), pmi_adj))  #两层aspect-focus GCN
        # x_pmi = F.relu(self.gc2(self.position_weight(x_pmi, aspect_double_idx, text_len, aspect_len), pmi_adj))
        # x_d_pmi = F.relu(self.gc3(self.position_weight(x_pmi, aspect_double_idx, text_len, aspect_len), dep_adj)) #两层inter-aspect GCN
        # x_d_pmi = F.relu(self.gc4(self.position_weight(x_d_pmi, aspect_double_idx, text_len, aspect_len), dep_adj))
        # x = 0.5 * x_pmi + 0.5 * x_d_pmi

        # x_cos = F.relu(self.gc5(self.position_weight(text_out, aspect_double_idx, text_len, aspect_len), cos_adj))  #两层aspect-focus GCN
        # x_cos = F.relu(self.gc6(self.position_weight(x_cos, aspect_double_idx, text_len, aspect_len), cos_adj))
        # x_d_cos = F.relu(self.gc7(self.position_weight(x_cos, aspect_double_idx, text_len, aspect_len), dep_adj)) #两层inter-aspect GCN
        # x_d_cos = F.relu(self.gc8(self.position_weight(x_d_cos, aspect_double_idx, text_len, aspect_len), dep_adj))
        # x2 = 0.5 * x_cos + 0.5 * x_d_cos
