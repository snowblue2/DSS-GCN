...
class ablation(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(num_layer):
            if i == 0:
                x_pmi_1 = F.relu(F.softmax(self.gc1(text_out, pmi_adj),dim=-1))
                x_pmi_2 = F.relu(F.softmax(self.gc2(x_pmi_1, pmi_adj),dim=-1))
                # x_cos_1 = F.relu(F.softmax(self.gc3(text_out,cos_adj),dim=-1))
                # x_cos_2 = F.relu(F.softmax(self.gc4(x_cos_1,cos_adj),dim=-1))
                ...
                x_d_pmi = F.relu(self.gc5(self.position_weight(x_pmi_2, aspect_double_idx, text_len, aspect_len), dep_adj))
                x_p_d = F.relu(self.gc6(self.position_weight(x_d_pmi, aspect_double_idx, text_len, aspect_len), dep_adj))
                # x_d_cos = F.relu(self.gc7(self.position_weight(x_cos_2, aspect_double_idx, text_len, aspect_len), dep_adj))
